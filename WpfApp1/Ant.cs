...
public class Ant: ISimulationAgent<AntState>
{
    ...
    private readonly int _depositPheromoneAfterSteps = 3;
    ...
    private int _stepsWithoutEvent = 0;
    private const int StepsWithoutEventBeforeReset = 5000;
    ...
    public void Act(SimulationArena<AntState> arena, float timeDelta)
    {
    ...
    private void Forage(SimulationArena<AntState> arena, List<(ISimulationResource, float, float, double)> food, float timeDelta)
    {
        ...
        if (_stepsWithoutEvent < StepsWithoutEventBeforeReset)
        {
            ...
            if (pheromonesWithDistanceAndOrientation.Count > 0)
            {
                _currentHighestPheromoneAmountSensed = new Some<float>(pheromonesWithDistanceAndOrientation
    ...
    private List<(ISimulationResource, float, float, double)> GetFoodInSensoryFieldWithDistanceAndSaliency(SimulationArena<AntState> arena)
    {
    ...
    private void DepositPheromonePeriodically(SimulationArena<AntState> arena)
    {
    ...
    private (float, float, float) GetOrientationAndPosDeltaAwayFromWalls(
        SimulationArena<AntState> arena, 
        float timeDelta,
    ...
    private void DepositPheromone(SimulationArena<AntState> arena)
    {
        ISimulationResource pheromone = 
            _mode == Mode.Forage ? 
                new PheromoneResource(State.X, State.Y, 0.2f, 0.15f) : 
                new PheromoneResourceReturn(State.X, State.Y, 0.2f, 0.15f);
