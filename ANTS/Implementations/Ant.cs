...
public class Ant : ISimulationAgent
{
    ...
    private const int StepsWithoutEventBeforeReset = 1100;
    ...
    private void DepositPheromone(ISimulationArena arena)
    {
        ...
        {
            case Mode.Forage:
                arena.AddPheromone("pheromone", new Point(State.X, State.Y), 0.25f, 0.05f);
                break;
            case Mode.Return:
                arena.AddPheromone("pheromone-r", new Point(State.X, State.Y), 0.25f, 0.05f);
                break;
            default:
